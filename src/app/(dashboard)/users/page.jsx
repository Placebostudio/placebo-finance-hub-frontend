...
const ROLE_VARIANTS = { admin: "destructive", finance: "default", employee: "secondary" };
    ...
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-md">
        ...
        <div className="space-y-3">
          <div><Label className="text-xs">Full Name *</Label><Input value={form.fullName} onChange={(e) => set("fullName", e.target.value)} /></div>
          ...
          <div><Label className="text-xs">{user ? "New Password (leave blank to keep)" : "Password *"}</Label><Input type="password" value={form.password} onChange={(e) => set("password", e.target.value)} /></div>
          <div>
  ...
  return (
    <div className="flex flex-col h-full">
      <PageHeader
        ...
        actions={
          <Button size="sm" onClick={() => { setEditingUser(null); setShowDialog(true); }}>
                ...
                <CardContent className="p-4 flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge variant={ROLE_VARIANTS[u.role] ?? "secondary"} className="text-xs">{u.role}</Badge>
                  ...
                  <div className="flex gap-1 flex-shrink-0">
                    <Button size="sm" variant="ghost" className="h-7 text-xs" onClick={() => handleToggle(u.id, u.isActive)}>
                      {u.isActive ? "Deactivate" : "Activate"}
                    </Button>
                    <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => { setEditingUser(u); setShowDialog(true); }}>
                      <Pencil className="h-3.5 w-3.5" />
                    </Button>
                    {u.id !== currentUser.id && (
                      <Button size="icon" variant="ghost" className="h-7 w-7 text-destructive hover:text-destructive" onClick={() => handleDelete(u.id)}>
